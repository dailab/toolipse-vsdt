...
/**
 * Transformation from VSDT-BPMN to Structured BPMN.
 * 
 * @author kuester
 */
public class Bpmn2StrucBpmnTransformation extends MappingStage {
	...
	protected boolean internalApply() {
		...
		/*
		 * The ordering of the structuring rules is of great importance, as it 
		 * can have effects on whether and how a rule is applied in a certain 
		 * situation. E.g., the block rule will be applied as soon as there are 
		 * two gateways connected with two branches, even if there are other 
		 * branches that belong to these two gateways but are not yet connected.
		 * Thus the block rule should be tried last, after e.g. the loop rule 
		 * and the event handler rules. 
		 */
		...
		Transformation.transform((EObject) wrapper.getSourceModel(), rules);
		return true;
	}
