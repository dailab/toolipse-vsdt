...
import de.dailab.vsdt.meta.diagram.part.VsdtDiagramUpdater;
import de.dailab.vsdt.meta.diagram.part.VsdtLinkDescriptor;
import de.dailab.vsdt.meta.diagram.part.VsdtNodeDescriptor;
...
public class BusinessProcessSystemCanonicalEditPolicy extends
	...
	protected List getSemanticChildrenList() {
		...
		for (Iterator it = VsdtDiagramUpdater
				.getBusinessProcessSystem_79SemanticChildren(viewObject)
				.iterator(); it.hasNext();) {
	...
	private Collection collectAllLinks(View view, Map domain2NotationMap) {
		if (!BusinessProcessSystemEditPart.MODEL_ID.equals(VsdtVisualIDRegistry
				.getModelID(view))) {
			return Collections.EMPTY_LIST;
		}
		...
		case BusinessProcessSystemEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(VsdtDiagramUpdater
						.getBusinessProcessSystem_79ContainedLinks(view));
			}
			if (!domain2NotationMap.containsKey(view.getElement())
		...
		case ParticipantEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(VsdtDiagramUpdater
						.getParticipant_1001ContainedLinks(view));
			}
			if (!domain2NotationMap.containsKey(view.getElement())
					|| view.getEAnnotation("Shortcut") == null) { //$NON-NLS-1$
				domain2NotationMap.put(view.getElement(), view);
		...
		case BusinessProcessDiagramEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(VsdtDiagramUpdater
						.getBusinessProcessDiagram_1002ContainedLinks(view));
			}
			if (!domain2NotationMap.containsKey(view.getElement())
					|| view.getEAnnotation("Shortcut") == null) { //$NON-NLS-1$
				domain2NotationMap.put(view.getElement(), view);
