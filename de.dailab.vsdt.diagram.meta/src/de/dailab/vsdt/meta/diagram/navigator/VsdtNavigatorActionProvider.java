...
import de.dailab.vsdt.meta.diagram.part.VsdtDiagramEditor;
	...
	private class OpenDiagramAction extends Action {
		...
		public final void selectionChanged(IStructuredSelection selection) {
			...
			if (selection.size() == 1) {
				...
				if (selectedElement instanceof VsdtNavigatorItem) {
					selectedElement = ((VsdtNavigatorItem) selectedElement)
							.getView();
				...
				if (selectedElement instanceof Diagram) {
					...
					if (BusinessProcessSystemEditPart.MODEL_ID
							.equals(VsdtVisualIDRegistry.getModelID(diagram))) {
						myDiagram = diagram;
					}
				}
			}
		...
		public void run() {
			...
			try {
				page.openEditor(editorInput, VsdtDiagramEditor.ID);
